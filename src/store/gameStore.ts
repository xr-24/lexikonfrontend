...
interface GameStore extends GameState, RoomState, UIState, ChatState, DamageState, SettingsState {
  ...
  // Damage actions
  addDamageEvent: (playerId: string, damage: number, isAI: boolean) => void;
...
const initialDamageState: DamageState = {
  damageEvents: [],
};
...
export const useGameStore = create<GameStore>((set, get) => {
  ...
  // Evocation response handlers
  socket.on('activate-evocation-response', (result: any) => {
    if (result.success) {
      set({ currentMoveErrors: [] });
    } else {
      set({ currentMoveErrors: [result.error || 'Failed to activate evocation'] });
    }
  });

  // Evocation notification handlers
  socket.on('evocation-activated', (data: any) => {
    // Play evocation sound when someone uses an evocation
    soundService.playEvocation(data.evocationType);
    console.log(`${data.playerName} activated evocation: ${data.evocationType}`);
    
    // Add chat message about evocation activation
    const { chatMessages } = get();
    const chatMessage = {
      id: Date.now().toString() + Math.random().toString(36).substr(2, 9),
      playerId: data.playerId,
      playerName: data.playerName,
      playerColor: '#8a2be2', // Purple color for evocations
      message: `activated evocation: ${data.evocationType}`,
      timestamp: new Date(),
    };
  ...
  socket.on('damage-event', (data: any) => {
    const { addDamageEvent } = get();
    addDamageEvent(data.playerId, data.damage, data.isAI);
  });
  ...
  return {
    ...
    // Damage actions
    addDamageEvent: (playerId: string, damage: number, isAI: boolean) => {
      const { damageEvents } = get();
      ...
      set({ damageEvents: [...damageEvents, newEvent] });
    ...
    removeDamageEvent: (eventId: string) => {
      const { damageEvents } = get();
      set({ damageEvents: damageEvents.filter(event => event.id !== eventId) });
    },
